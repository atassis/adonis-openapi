...
import _, { snakeCase, startCase } from 'lodash';
...
export class CommentParser {
  ...
  constructor(options: AdonisOpenapiOptions) {
...
export function parseModelProperties(_snakecase: boolean, data) {
  ...
  lines.forEach((line, index) => {
    ...
    if (example === null || example === 'string') {
      example = exampleGenerator.exampleByField(field) || exampleGenerator.exampleByType(type);
    }
  ...
  async parsePropsAndMeta(obj, testObj, validator: VineValidator<any, any>) {
    ...
    for (const m of msgs) {
      ...
      if (err === 'TYPE') {
        ...
        _.set(testObj, m.field, this.exampleGenerator.exampleByType(m.rule));
      }
      ...
      if (err === 'FORMAT') {
        ...
        _.set(testObj, m.field, this.exampleGenerator.exampleByValidatorRule(m.rule));
      }
    }
