...
async function generate(adonisRoutes: AdonisRoutes, options: AdonisOpenapiOptions) {
  const normalizedOptions = {
    ...
    ...options,
    appPath: `${options.path}app`,
  };

  const routes = adonisRoutes.root;
  const customPaths: CustomPaths = {};

  try {
    const pj = await readFile(join(normalizedOptions.path, 'package.json'));

    const pjson = JSON.parse(pj.toString());
    if (pjson.imports) {
      Object.entries(pjson.imports).forEach(([key, value]) => {
        const k = (key as string).replaceAll('/*', '');
        customPaths[k] = (value as string).replaceAll('/*.js', '').replaceAll('./', '');
      });
    }
  } catch (e) {
    console.error(e);
  ...
  const schemas = await getSchemas(customPaths, {
    snakeCase: true,
    preferredPutPatch: 'PUT',
    debug: false,
    ...normalizedOptions,
  ...
  if (normalizedOptions.debug) {
    ...
    console.log('Using custom paths', customPaths);
  }

  const docs = {
    openapi: '3.1.0',
    info: normalizedOptions.info,
    components: {
      responses: {
        Forbidden: {
      ...
      securitySchemes: {
        ...
        ...normalizedOptions.securitySchemes,
      },
      schemas,
    },
    paths: {},
  ...
  if (normalizedOptions.defaultSecurityScheme) {
    sscheme = normalizedOptions.defaultSecurityScheme;
  }

  const securities = {
    auth: { [sscheme]: ['access'] },
    'auth:api': { [sscheme]: ['access'] },
    ...normalizedOptions.authMiddlewares?.reduce((acc, am) => {
  ...
  for await (const route of routes) {
    let ignore = false;
    for (const i of normalizedOptions.ignore) {
      if (
        route.pattern === i ||
    ...
    let { tags, parameters, pattern } = extractRouteInfos(normalizedOptions, route.pattern);
    ...
    route.methods.forEach((method) => {
      ...
      if (
        ...
        method !== normalizedOptions.preferredPutPatch
      )
        return;
      ...
      if (action !== '' && summary === '') {
        ...
        // https://github.com/atassis/adonisjs-openapi/issues/28
        tags[0] = tags[0] ?? '';
  ...
  docs.paths = paths;
  if (options.onPreGenerate) {
    return options.onPreGenerate(docs);
  }
  return docs;
}
 ...
 */
export async function generateOpenapiJson(routes: any, options: AdonisOpenapiOptions) {
  if (process.env.NODE_ENV === (options.productionEnv || 'production')) {
    const str = await readLocalFile(options.path, 'json');
    return JSON.parse(str);
  }
  return await generate(routes, options);
}

/**
 * Export OpenAPI file specification
 */
...
export async function generateOpenapiYaml(routes: any, options: AdonisOpenapiOptions) {
  ...
  return jsonToYaml(await generate(routes, options));
}
